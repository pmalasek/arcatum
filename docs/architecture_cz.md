...
- ✓ **Manuální trigger** z web UI („run now") i přes API.
...
**HTTP API (`internal/server`)** — kompletní přehled v [README](../README_cz.md#http-api).
...
**Vyzkoušení lokálně:** viz [README — Rychlý start](../README_cz.md#rychlý-start-lokální-vyzkoušení)
a [Zabezpečení](../README_cz.md#zabezpečení-mtls-a-podpis-úloh).
...
**Neověřeno:** vzhled záložky Restore v prohlížeči — v tomto prostředí není headless
...
obrazovka a záložka Users jsou ověřené jen na úrovni API a servírování assetů.
...
Sloupec **Repository** ukazuje u streamované instance `<velikost> · N dumpů` místo `—`
(`storedSummary`), takže instance, která má každou zálohu, jakou si vyžádala, už
nevypadá jako instance bez záloh. Záložka **Restore** u ní vypíše dumpy ke stažení místo
...
nasbíraného. Obojí obsluhuje záložka **Administration** a obojí je `admin only`.
