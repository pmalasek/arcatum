...
postavené „run now" i živý tail ve webu.
...
patří do instance (viz [README → Skript vs. instance](../README_cz.md#skript-vs-instance)).
...
Je-li po ruce [`just`](../README_cz.md#zkratky-přes-just), je tohle kolo `just trigger
...
[návodu na backend](backend-development_cz.md#2-lokální-vývojová-smyčka).

### c) Nakonec na cílovém stroji

Skript, který závisí na prostředí (verze `mysqldump`, práva, dostupnost socketu), musí
projít **na tom hostu, kde má běžet** — s ostrým runnerem a přes web „run now".
...
> [`just`](../README_cz.md#zkratky-přes-just) tuhle past obcházejí: `just run-output 1`
...
| `status` = `cancelled` | běh zastavil operátor tlačítkem **stop**; není co vyšetřovat |
...
- [ ] u souborové zálohy: **obnova jednoho souboru vyzkoušená** ze záložky Restore
- [ ] manifest i kód commitnuté (instance a hesla ne — ty jsou v DB)

Související: [README → Jak napsat vlastní zálohovací skript](../README_cz.md#jak-napsat-vlastní-zálohovací-skript) ·
[vývoj backendu](backend-development_cz.md) · [nasazení produkce](production_cz.md)
